...
Nótese que esta consulta requiere que por la parte de la aplicación se filtre por los 5 mejores tiempos. Esto es debido a que, aunque la tabla está ordenada por tiempo, no se puede limitar el número de resultados por mazmorra devueltos por Cassandra. Por otro lado, la consulta se realiza con 
un nivel de consistencia de uno, ya que la consistencia en este *leaderboard* no es tan importante y, en cambio, el rendimiento es crítico.

#### Lógica de las escrituras

La lógica de las escrituras pretende a
...
Es importante que las lecturas que realicemos en este proceso sean consistentes para evitar añadir a un mismo usuario más de una vez en el ranking

Como se ha mencionado anteriormente, debemos utilizar un nivel de consistencia de `QUORUM`, ya que las escrituras se realizan con un nivel de consistencia de `QUORUM`. Es necesario remarcar esto, porque es importante que las lecturas que realicemos en este proceso sean consistentes para evitar añadir a un mismo usuario más de una vez en el ranking.
...
Opcionalmente, una vez se hayan completados estos pasos, se podría eliminar también del ranking el usuario que 
ha sido desbancado. No obstante, esto no es estrictamente necesario, ya que la aplicación debe de filtrar los resultados 
para mostrar únicamente los 5 mejores tiempos.
